...
from app.models import ContentItem, ContentStatus, ScheduledPost
...
def schedule_one(req: ScheduleRequest, db: Session = Depends(get_db)):
...
def auto_schedule(req: AutoScheduleRequest, db: Session = Depends(get_db)):
...
def unschedule(schedule_id: int, db: Session = Depends(get_db)):
